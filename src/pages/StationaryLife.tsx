...
import { formatMs, formatMsShort, formatDateKey } from "../lib/storage";
...
const StationaryLife = () => {
  ...
  const weeklyAvg = useMemo(() => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const m = i === 0 ? todayMetrics : getMetricsForDate(k);
      if (m.stationaryMs > 0) {
        total += m.stationaryMs;
        count++;
      }
    }
    return count > 0 ? Math.round(total / count) : 0;
  }, [todayMetrics, getMetricsForDate]);
        ...
        <div className="max-w-md mx-auto w-full flex flex-col items-center justify-center min-h-[70vh]">
          ...
          <div className="mb-8">
            <div className="w-32 h-32 rounded-full bg-linear-to-br from-[#131F37]/85 to-[#0F172A]/85 border border-white/10 flex items-center justify-center">
              <Armchair
                className="w-16 h-16 text-[#4B6FA7]"
                strokeWidth={1.5}
              />
            </div>
          </div>
          ...
          <div className="text-center mb-8">
            <p className="text-4xl font-bold text-[#F8FAFC] mb-2">
              {formatMs(todayMetrics.stationaryMs)}
            </p>
            <p className="text-sm text-[#94A3B8]">
              Tiempo en dispositivo sin movimiento
            </p>
          </div>
          ...
          <div className="w-full bg-linear-to-br from-[#131F37]/85 to-[#0F172A]/85 rounded-2xl p-6 border border-white/10">
            ...
            <p className="text-sm text-[#94A3B8] leading-relaxed mb-4">
              Esta métrica registra el tiempo que pasas usando tu dispositivo
              mientras estás sentado o sin moverte significativamente.
            </p>
            <p className="text-sm text-[#94A3B8] leading-relaxed">
              Un tiempo prolongado de vida estacionaria puede afectar tu salud.
              Considera tomar descansos regulares y moverte más.
            </p>
          </div>
          ...
          <div className="w-full mt-6 bg-[#4B6FA7]/10 rounded-xl p-4 border border-[#4B6FA7]/20">
            <p className="text-xs text-[#94A3B8] text-center">
              💡 <span className="font-semibold">Consejo:</span> Levántate cada
              30-60 minutos para estirar y moverte
            </p>
          </div>
          ...
          <div className="w-full mt-4 grid grid-cols-2 gap-3">
            <div className="bg-[#1E293B] rounded-xl p-4 text-center">
              ...
              <p className="text-lg font-bold text-[#F8FAFC]">
                {formatMsShort(todayMetrics.stationaryMs)}
              </p>
            </div>
            <div className="bg-[#1E293B] rounded-xl p-4 text-center">
              <p className="text-xs text-[#94A3B8] mb-1">Promedio semanal</p>
              <p className="text-lg font-bold text-[#F8FAFC]">
                {formatMsShort(weeklyAvg)}
              </p>
            </div>
          </div>
        </div>
