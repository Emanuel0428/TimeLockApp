...
const ContinuousUse = () => {
  ...
  const displayMax = isToday
    ? Math.max(dayMetrics.continuousMaxMs, currentStreakMs)
    : dayMetrics.continuousMaxMs;
        ...
        <div className="max-w-md mx-auto w-full">
          ...
          <div className="mb-4 text-center">
            <p className="text-sm text-[#94A3B8]">
              {displayMax > 0
                ? `Racha máxima: ${formatMs(displayMax)}`
                : "No hay registros hoy"}
            </p>
            {isToday && currentStreakMs > 0 && (
              <p className="text-xs text-[#4B6FA7] mt-1">
                Racha actual: {formatMs(currentStreakMs)}
              </p>
            )}
          </div>
          ...
          <div className="bg-linear-to-br from-[#131F37]/85 to-[#0F172A]/85 rounded-2xl p-6 border border-white/10">
            <h3 className="text-base font-semibold text-[#F8FAFC] mb-3 text-center">
              Uso Continuo
            </h3>
            <p className="text-sm text-center text-[#94A3B8] leading-relaxed mb-4">
              El tiempo más largo que usaste tu dispositivo sin interrupciones
              significativas.
            </p>
            <div className="bg-[#4B6FA7]/10 rounded-lg p-3 border border-[#4B6FA7]/20">
              <p className="text-xs text-[#94A3B8] text-center">
                💡 Tomar descansos regulares ayuda a reducir la fatiga visual y
